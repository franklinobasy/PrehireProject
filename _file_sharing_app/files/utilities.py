...
def check_file_permissions(user, file):
    ...
    return (
        file.uploaded_by == user or
        UserFilePermission.objects.filter(user=user, shared_file=shared_file).exists() or
        TeamFilePermission.objects.filter(shared_file=shared_file, team__in=user.teams.all()).exists()
    )