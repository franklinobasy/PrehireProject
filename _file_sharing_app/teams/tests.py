...
class TeamTests(APITestCase):
    ...
    def test_add_member_user_not_found(self):
        ...
        add_member_url = reverse('add-member', args=[team.id])  # Update with correct URL
        non_existent_user_id = 9999
        response = self.client.post(add_member_url, {'user_id': non_existent_user_id}, format='json')
    ...
    def test_remove_member(self):
        ...
        remove_member_url = reverse('remove-member', args=[team.id])  # Update with correct URL
        response = self.client.post(remove_member_url, {'user_id': other_user.id})
        self.assertEqual(response.status_code, 200)  # Check if member was removed successfully
    ...
    def test_remove_member_user_not_in_team(self):
        ...
        remove_member_url = reverse('remove-member', args=[team.id])  # Update with correct URL
        response = self.client.post(remove_member_url, {'user_id': other_user.id})
        self.assertEqual(response.status_code, 400)  # Check if user not found in the team response is returned

    def test_update_team(self):
        # Create a team first
        update_team_url = reverse('update-team', args=[self.team.id])  # Update with correct URL
        response = self.client.post(update_team_url, {'name': 'Updated Team', 'description': 'Updated description'})
        self.assertEqual(response.status_code, 200)  # Check if team update was successful

    def test_delete_team(self):
        delete_team_url = reverse('delete-team', args=[self.team.id])  # Use correct URL for delete
